...
<Steps>
  <Step title="Ground">
    `list_content`, then `get_content` on the best few (patterns), `get_brand_kit` (voice and visual), `search_brand_knowledge` (your stance), and `get_account` (your own top posts).
  </Step>
  <Step title="Draft">
  ...
  <Step title="Assemble">
    `create_card`, `update_card` with `assets` (the video), `posts` (platform, format, connected account).
  </Step>
  <Step title="Publish">
    `update_card` with `scheduledAt`, after confirming destinations and timing.
  </Step>
</Steps>
