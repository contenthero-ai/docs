...
The library: browse, search, import and upload the media an account owns. 14 tools.
...
The media to LOOK at, up to 5 per call. Five is where the inline byte budget runs out, not a round number. Use show_media for a larger set you only need to display.
</ParamField>
...
<ParamField body name="items" type="value[]" required>
The media to show, up to 100 items per call.
</ParamField>
