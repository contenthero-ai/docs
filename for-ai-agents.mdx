...
The fastest way to drive ContentHero well is to install the official skill, which encodes the workflows and our model-specific prompt craft. In a terminal agent, paste this. In a chat app, follow [Install the Skill](/skills/install) instead: chat apps install by upload.
...
The skill repo is [`contenthero-ai/skills`](https://github.com/contenthero-ai/skills). It holds ONE skill, `contenthero`, covering the whole product: generating media, the planner and publishing, brand context and its knowledge base, research by outlier score, the media library, and the editor. Its CI checks that every tool this reference advertises is reachable through one of its workflows, so the skill cannot fall behind the API.

## Async, chaining, and scopes

- **Async generation:** `generate_*` runs a smart-wait (about 50s) then hands back an `outputId` if the render is still going. Poll with `get_generation_status`, which blocks until terminal by default; pass `wait: false` for a snapshot. The `outputId` is always returned, so keep polling.
