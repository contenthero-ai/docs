...
<Tabs>
  <Tab title="MCP">
    ...
    Your host walks you through the OAuth consent once. After that, the ContentHero tools appear as `mcp__contenthero__*`. See [MCP setup](/tools/mcp) for per-host instructions.
  </Tab>
  <Tab title="CLI">
    ```bash
    ...
    The CLI stores the key at `~/.contenthero/credentials` (mode 0600). You can also set `CONTENTHERO_API_KEY` in the environment. See [CLI setup](/tools/cli).
  </Tab>
  <Tab title="Raw API">
    ```bash
...
<CardGroup cols={2}>
  ...
  <Card title="Install the skills" icon="robot" href="/for-ai-agents">
    Let your agent drive the whole surface with the official workflows.
  </Card>
  <Card title="Browse the recipes" icon="book-open" href="/cookbook">
