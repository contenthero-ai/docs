...
1. **Never generate copy as a black box.** ContentHero's servers do not write copy. You may draft, but only grounded in the user's real context (outliers plus brand voice plus past posts), never generic, and never published without explicit approval. See [the thesis](/#the-thesis).
